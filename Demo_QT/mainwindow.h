...
public:
    ...
    void generate_points(int number_of_points);
...
public:
    ...
    void afisareVect(Nod* nod, vector<NodAfis> &vectAfis) {
        ...
        if (nod != nullptr) {
            ...
            if (nod->stanga != nullptr || nod->dreapta != nullptr) {
                ...
                if (nod->stanga != nullptr)
                {
                    ...
                    vectAfis.push_back({ nod->stanga->valoare, nod->stanga->ordine, nod->stanga->nivel,nod->ordine,nod->nivel});
                ...
                if (nod->dreapta != nullptr)
                {
                    ...
                    vectAfis.push_back({ nod->dreapta->valoare, nod->dreapta->ordine, nod->dreapta->nivel,nod->ordine,nod->nivel });
            ...
            afisareVect(nod->stanga,vectAfis);
            afisareVect(nod->dreapta,vectAfis);
        }
    }
    ...
    vector<NodAfis> afisareVect() {
        vector<NodAfis> vectAfis;
        vectAfis.push_back({radacina->valoare,1,1});
        afisareVect(radacina,vectAfis);
        return vectAfis;
    }

    void afisareVector()
    {
        vector<NodAfis> vect = afisareVect();
        for (int i = 0; i < vect.size(); i++)
            cout << vect[i].valoare << " " << vect[i].nivel << " " << vect[i].ordine << endl;
    }
};
